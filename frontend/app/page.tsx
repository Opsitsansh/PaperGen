...
// ✅ FIXED: removed trailing slash at the end
const BACKEND_URL = "https://ansh-production-ae92.up.railway.app";
// ------------------------------------
...
export default function Home() {
  ...
  // Helper text for file display
  const getFileDisplay = () => {
    if (files.length === 0) return "";
    if (files.length === 1)
      return files[0].name.replace(/\.(pdf|png|jpg|jpeg|webp)$/i, "");
  ...
  return (
    ...
    >
      ...
      {/* PAGE 1: LANDING */}
      <div
        className={`fixed inset-0 w-full h-full flex flex-col items-center justify-center transition-all duration-1000 ease-in-out z-50 bg-black/80 backdrop-blur-sm ${
          view === "landing"
      ...
      <div
        className={`absolute inset-0 flex items-center justify-center p-6 transition-all duration-700 ${
          view === "upload"
            ...
            : "opacity-0 -translate-x-20 pointer-events-none z-0"
        }`}
      >
      ...
      >
        <div className="w-full max-w-4xl bg-zinc-900 border border-zinc-800 rounded-3xl flex flex-col h-[85vh] shadow-2xl overflow-hidden relative">
            ...
            <div className="flex items-center gap-3">
              ...
              {mode !== "Chat with PDF" && generatedContent && (
                <button
                  onClick={handleSpeak}
                  title="Read Aloud"
                  className={`p-2 rounded-lg transition-all border border-zinc-700 ${
                    isSpeaking
          ...
          {/* CONTENT AREA */}
          <div
            className="flex-1 overflow-auto p-8 bg-black relative custom-scrollbar"
            ref={chatScrollRef}
          >
            {/* --- CHAT VIEW --- */}
            {mode === "Chat with PDF" ? (
              <div className="flex flex-col gap-6">
                {messages.length === 0 && (
                  <TextGenerateEffect
                    words="Ask me anything about your documents..."
                    className="text-zinc-500 text-center mt-20"
                  />
                )}
            ...
            generatedContent ? (
              <div
                ref={documentRef}
                className="bg-white text-black p-12 shadow-2xl min-h-full w-full max-w-[210mm] mx-auto rounded-sm"
              >
              ...
              <div className="h-full flex flex-col items-center justify-center text-zinc-600 space-y-4">
                ...
                <TextGenerateEffect
                  ...
                  className="text-zinc-400 text-xl font-medium text-center max-w-lg"
                />
              </div>
            )}
          </div>
